<template>
  ...
  <div class="flex flex-col justify-center items-center p-12">
    ...
    <div class="mt-4 w-[100%] flex flex-col justify-center items-center">
      <p class="text-3xl  text-primary mb-16 font-medium">
        {{ capitalize(title) }}
      </p>
      <!-- CONTENT-CONTAINER -->
      <div class="flex flex-row justify-center items-center w-[100%] gap-x-60">
        ...
        <div class="flex flex-col justify-center items-center gap-4 ">
          <div
            ...
            :key="cipherType"
            class=" text-xl select-none cursor-pointer"
        ...
        <div class="bg-secondary rounded-2xl flex flex-col p-6 py-10 justify-center items-center">
          ...
          />
          <div class="flex flex-row text-secondary border-current border-[2px] rounded-md py-2 px-4 gap-x-2 select-none cursor-pointer w-[90%] justify-center items-center mt-10 hover:bg-current" @click="handleStartButton">
            <p class="text-primary  font-semibold text-base">
              {{ capitalize(menuState.activeMode) }}
            </p>
          </div>
        </div>
      </div>
        ...
        <div class="flex flex-row justify-center items-center bg-secondary rounded-xl" :class="[menuState.activeType !== TYPE.EXTENDED_VIGENERE ? 'w-[100%] p-10' : 'p-4']">
              ...
              >
                <div class="flex flex-row justify-center items-center gap-x-8 mb-4">
                  <p class=" font-semibold text-xl text-primary">
                    {{ capitalize(resultLabel) }}
                  </p>
                  <div class="text-secondary border-[2px] border-current select-none cursor-pointer hover:bg-current rounded-md px-3 py-2" @click="() => handleDownload(resultLabel == resultTypes.NO_SPACE ? outputTypes.NO_SPACE : outputTypes.GROUPED)">
                    <p class=" text-primary text-sm">
                      Download
                    </p>
                  </div>
                </div>
                <p class=" text-alternate text-lg">
                  Lorem ipsum dolor sit amet consectetur adipisicing elit. Facilis recusandae praesentium dolor odit tempora doloremque iusto vitae ratione deleniti iure nemo sint aliquam repudiandae excepturi aut tenetur, nesciunt quod neque.
                </p>
          ...
          <template v-else>
            <div class="flex flex-col justify-center items-center gap-6">
              ...
              <div class="text-secondary border-[2px] border-current select-none cursor-pointer hover:bg-current rounded-md px-3 py-2" @click="() => handleDownload(outputTypes.FILE)">
                <p class=" text-primary text-sm">
                  Download
                </p>
              </div>
...
import { MODE, TYPE, RESULT_STRING_TYPE, RESULT } from '~/constants'
...
const textFileRef: Ref<string|any> = ref(null)
const keyFileRef: Ref<string|any> = ref(null)
